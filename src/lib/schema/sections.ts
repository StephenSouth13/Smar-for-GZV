...
export const imageGalleryDataSchema = z.object({
  heading: z.string().default(""),
  images: z
    .array(z.object({ imageUrl: z.string().min(1), caption: z.string().default("") }))
    .default([]),
});
...
// Defined via mapped type + index access so it collapses into a proper
// discriminated union (narrowing `type` narrows `data`), unlike a plain
// generic `Section<T>` instantiated at `SectionType` which does not.
export type Section = {
  [K in SectionType]: { id: string; type: K; order: number; data: SectionDataMap[K] };
}[SectionType];
...
export const SECTION_TYPE_META: Record<SectionType, { label: string; description: string }> = {
  ...
  logoGrid: { label: "Lưới logo khách hàng", description: "Hiển thị logo các khách hàng/đối tác" },
  projectGrid: { label: "Lưới dự án", description: "Danh sách dự án nổi bật (tự động hoặc chọn tay)" },
  articleGrid: { label: "Lưới bài viết", description: "Danh sách bài viết mới nhất (tự động hoặc chọn tay)" },
  cta: { label: "Kêu gọi hành động (CTA)", description: "Khối nhấn mạnh thông điệp kèm nút bấm" },
  ...
  stats: { label: "Số liệu nổi bật", description: "Các con số ấn tượng (năm kinh nghiệm, khách hàng...)" },
...
export function createSection<T extends SectionType>(type: T, order: number, id: string): SectionOf<T> {
  return { id, type, order, data: defaultSectionData(type) } as unknown as SectionOf<T>;
}
