        ...
        <ListEditor
          ...
          renderItem={(item, update) => (
            <div className="space-y-3">
              <ImageField label="Logo" value={item.imageUrl} onChange={(url) => update({ imageUrl: url })} />
              <div className="grid grid-cols-2 gap-3">
                <Input placeholder="Tên khách hàng" value={item.name} onChange={(e) => update({ name: e.target.value })} />
                <Input placeholder="Liên kết (tuỳ chọn)" value={item.link} onChange={(e) => update({ link: e.target.value })} />
              </div>
            </div>
          )}
        />
